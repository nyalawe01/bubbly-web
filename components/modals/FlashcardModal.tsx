...
export function FlashcardModal({ open, onClose, onGenerate, colors, uploadedFiles = [], onFileUpload, vaultDocuments = [] }: FlashcardModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      ...
      >
        <div className={`flex items-center justify-between border-b ${colors.borderBase} px-6 py-4`}>
          <div className="flex items-center gap-3">
            {showSources ? (
              ...
              <Layers className={`h-5 w-5 ${colors.textSecondary}`} />
            )}
            <h2 className={`text-lg font-semibold ${colors.textPrimary}`}>{showSources ? "Sources" : "Flashcards"}</h2>
          </div>
        ...
        {!showSources ? (
          <div className="p-6">
            <div className="flex flex-wrap gap-8">
              <div>
                <p className={`mb-2 text-sm font-medium ${colors.textSecondary}`}>Number of Cards</p>
                <div className={`flex rounded-full border ${colors.borderBase} ${colors.bgInput} p-0.5`}>
                  {countOptions.map((option) => (
                    <button key={option.value} onClick={() => setCount(option.value)} className={`icon-motion rounded-full px-4 py-1.5 text-sm font-medium transition-colors ${count === option.value ? `${colors.btnPrimary} shadow-sm` : `${colors.textSecondary} ${colors.bgHover}`}`}>
                      {option.label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className={`flex rounded-full border ${colors.borderBase} ${colors.bgInput} p-0.5`}>
                  {difficultyOptions.map((option) => (
                    <button key={option.value} onClick={() => setDifficulty(option.value)} className={`icon-motion rounded-full px-4 py-1.5 text-sm font-medium transition-colors ${difficulty === option.value ? `${colors.btnPrimary} shadow-sm` : `${colors.textSecondary} ${colors.bgHover}`}`}>
                      {option.label}
                    </button>
                  ))}
                </div>
        ...
        <div className={`flex justify-end border-t ${colors.borderBase} px-6 py-4`}>
          <button onClick={() => onGenerate({ count, difficulty, topic, files, sourceIds: selectedVaultIds })} className={`icon-motion rounded-full border ${colors.borderBase} ${colors.bgInput} px-6 py-2 text-sm font-medium ${colors.textPrimary} hover:${colors.bgHover}`}>
            Generate
          </button>
        </div>
      </div>
