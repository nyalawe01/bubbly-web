...
import { X, ChevronDown, BrainCircuit, UploadCloud, Paperclip } from "lucide-react";
...
export function QuizModal({ open, onClose, onGenerate, colors, uploadedFiles = [], onFileUpload, vaultDocuments = [] }: QuizModalProps) {
        ...
        <div className={`flex items-center justify-between border-b ${colors.borderBase} px-6 py-4`}>
          <div className="flex items-center gap-3">
            {showSources ? (
              ...
              <BrainCircuit className={`h-5 w-5 ${colors.textSecondary}`} />
            )}
            <h2 className={`text-lg font-semibold ${colors.textPrimary}`}>{showSources ? "Sources" : "Quiz"}</h2>
          </div>
